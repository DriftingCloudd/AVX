...
static inline int
test_bit(unsigned int nr, const volatile uint64 *addr)
{
	return 1UL & (addr[BIT_WORD(nr)] >> (nr & (BITS_PER_LONG-1)));
}

static inline void
clear_bit(unsigned int nr, uint64 *addr)
{
...
sys_pselect6(void)
{
  int nfds;
	...
	if (uwfds && either_copyin((void *)&wfds, 1, uwfds, sizeof(struct fdset)) < 0)
		return -1;
	if (uexfds && either_copyin((void *)&exfds, 1, uexfds, sizeof(struct fdset)) < 0)
		return -1;
	if (utoaddr && either_copyin((void *)&timeout, 1, utoaddr, sizeof(timeout)) < 0)
		return -1;
	if (usmaddr && either_copyin((void *)&sigmask, 1, usmaddr, sizeof(sigmask)) < 0)
		return -1;
