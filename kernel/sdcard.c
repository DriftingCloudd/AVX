...
static void sd_read_data_dma(uint8 *data_buff, uint32 length) {
  ...
  spi_receive_data_standard_dma(-1, DMAC_CHANNEL0, SPI_DEVICE_0,
                                SPI_CHIP_SELECT_3, NULL, 0, data_buff, length);
}
 ...
 */
static void sd_get_response_R3_rest(uint8 *frame) { sd_read_data(frame, 4); }

/*
 * Read the rest of R7 response
 * Be noticed: frame should be at least 4-byte long
 */
static void sd_get_response_R7_rest(uint8 *frame) { sd_read_data(frame, 4); }
