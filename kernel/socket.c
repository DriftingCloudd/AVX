...
uint16 htons(uint16 x) { return (x >> 8) | (x << 8); }

uint16 ntohs(uint16 x) { return (x >> 8) | (x << 8); }
...
int do_accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
  ...
  release(&sock_lock);
  int new_sock_fd = do_socket(sock[sock_num].domain, sock[sock_num].type,
                              sock[sock_num].protocol);
  acquire(&sock_lock);
...
ssize_t do_sendto(int sockfd, void *buf, size_t len, int flags,
  ...
  printf("do_sendto dest sock: %d %d %d\n", dest_addr->sin_family,
         dest_addr->sin_port, dest_addr->sin_addr.s_addr);
  // for here, sockfd and flags are trivial because of the leak of network
  // function
  struct proc *curr_proc = myproc();
  if (curr_proc->ofile[sockfd]->type != FD_SOCK) {
    return -ENOTSOCK;
  ...
  // printf("ready to write ringbuffer! ringbuffer:%lx, buf = %lx, len =
  // %d\n",&sock[i].data,buf,len);
  int old_len = len;
  while (len > 0) {
...
ssize_t do_recvfrom(int sockfd, void *buf, size_t len, int flags,
  ...
  // printk("ready to read ringbuffer! ringbuffer:%lx, buf = %lx, len =
...
int do_getsockname(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
  ...
  copyout(curr_proc->pagetable, (uint64)addr, (char *)&sock[sock_num].addr,
