...
sys_openat()
{
  ...
  if (NULL == (ep = new_ename(dp,path))) {
    // 如果文件不存在
    if ((flags & O_CREATE) || strncmp(path,"/proc/loadavg",13) == 0 || strncmp(path,"/tmp/testsuite-",15) == 0 ||strncmp(path,"/dev/zero",9) == 0 ||strncmp(path,"/etc/passwd",11) == 0 ||strncmp(path,"/proc/meminfo",13) == 0 || strncmp(path,"/dev/tty",8) == 0 || strncmp(path,"/etc/localtime",14) == 0 || strncmp(path,"/dev/misc/rtc",13) == 0 || strncmp(path,"/proc/mounts",12) == 0 || strncmp(path, "/dev/urandom", 12) == 0) {
...
sys_copy_file_range(void)
{
  ...
  printf("enter here!\n");
  if (argfd(0, &fd_in, &fp_in) < 0 || argaddr(1, &off_in) < 0 || argfd(2, &fd_out, &fp_out) < 0 || argaddr(3, &off_in) < 0 || argint(4, &len) < 0)
  {
    return -1;
  }
  if (len == 0)
